...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-gray-200 p-4">
        ...
        <div class="lg:w-2/4 mx-auto py-8 px-6 bg-white rounded-xl mt-1">
            ...
            <div class="mb-6"> 
                <form class="flex flex-col space-y-4" method="POST" action="/">
                    ...
                    <div class="text-center text-xl">
                        <button class="w-64 h-16 py-4 px-8 bg-green-500 text-white rounded-xl font-bold">
                            Add-Items
                        </button>
                    </div>
                </form>
                    ...
                    <div @class([
                        ...
                        <div class="flex space-x-3">
                            ...
                            <form method="POST" action="/{{ $todo->id }}">
                                @csrf
                                @method('PATCH')
                                <button class="py-2 px-2 bg-green-500 text-white rounded-xl" title="{{ $done }}">
                            ...
                            <form method="POST" action="/{{ $todo->id }}">
                                @method('PUT')
                                <button class="py-2 px-2 bg-blue-500 text-white rounded-xl" title="{{ $edit }}">
                            ...
                            <form method="POST" action="/{{ $todo->id }}">
                                @csrf
                                @method('DELETE')   
                                <button class="py-2 px-2 bg-red-500 text-white rounded-xl" title="{{ $delete }}">
